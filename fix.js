...
const scanSchema = require('./scan').scan;

const { ValidatorResult } = helpers;
const { ValidatorResultError } = helpers;
const { SchemaError } = helpers;
const { SchemaContext } = helpers;
...
Validator.prototype.validate = function validate(
    ...
    const base = urilib.resolve(options.base || anonymousBase, id || '');
    if (!ctx) {
...
Validator.prototype.resolve = function resolve(schema, switchSchema, ctx) {
    ...
    const parsed = urilib.parse(switchSchema);
    const fragment = parsed && parsed.hash;
    const document =
        fragment &&
        fragment.length &&
    ...
    const subschema = helpers.objectGetPath(
